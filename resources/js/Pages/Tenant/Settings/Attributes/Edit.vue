  ...
  <div class="min-h-screen bg-slate-950 text-slate-100">
    <nav class="border-b border-slate-800/80 bg-slate-900/60 backdrop-blur-md sticky top-0 z-50">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-4 h-16">
        ...
        <p class="text-white font-bold text-sm">Editar Atributo Dinámico</p>
      </div>
    </nav>
        ...
        <form @submit.prevent="submit" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input v-model="form.name" type="text" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-fuchsia-500 focus:ring-1 focus:ring-fuchsia-500 transition" required />
...
const form = useForm({
  ...
  category_ids: props.attribute.categories ? props.attribute.categories.map(c => c.id) : []
})

onMounted(() => {
  if (form.options && form.options.length > 0) {
    optionsString.value = form.options.join(', ')
  }
})
...
const submit = () => {
  form.put(route('tenant.settings.attributes.update', props.attribute.id))
}
</script>
