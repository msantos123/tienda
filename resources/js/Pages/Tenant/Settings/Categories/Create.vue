  ...
  <div class="min-h-screen bg-slate-950 text-slate-100">
    <nav class="border-b border-slate-800/80 bg-slate-900/60 backdrop-blur-md sticky top-0 z-50">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-4 h-16">
        ...
        <p class="text-white font-bold text-sm">Nueva Categoría</p>
      </div>
    </nav>
      ...
      <div class="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6 shadow-xl">
        <form @submit.prevent="submit" class="space-y-6">
          <div>
            ...
            <input v-model="form.name" type="text" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition" placeholder="Ej. Laptops" required />
            <p v-if="form.errors.name" class="mt-2 text-sm text-rose-400">{{ form.errors.name }}</p>
          </div>
          
          <div>
            <label class="block text-sm font-medium text-slate-300 mb-2">Slug (Opcional)</label>
            <input v-model="form.slug" type="text" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition font-mono text-sm" placeholder="Se generará automáticamente si se deja en blanco" />
...
const submit = () => {
  form.post(route('tenant.settings.categories.store'))
}
</script>
